    ...
    if (tid === '七猫') {
      ...
      if (tokenRes?.data?.token) {
        ...
        if (res?.list) videos.push(...res.list.map(i => ({
          ...
          vod_name: i.title,
          vod_pic: `https://free-api.bighotwind.cc/papaya/papaya-file/files/download/${i.imageKey}/${i.imageName}`,
    ...
    if (platId === '七猫') {
      ...
      if (tokenRes?.data?.token) {
        ...
        if (res) {
          ...
          const pic = `https://free-api.bighotwind.cc/papaya/papaya-file/files/download/${d.imageKey}/${d.imageName}`;
          const u = (d.episodesList || []).map(e => {
            const best = e.resolutionList?.sort((a, b) => b.resolution - a.resolution)[0];
            return best ? `第${e.episodes}集$https://free-api.bighotwind.cc/papaya/papaya-file/files/download/${best.fileKey}/${best.fileName}` : null;
          }).filter(Boolean).join('#');
