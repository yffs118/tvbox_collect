...
class Spider(Spider):
    ...
    def playerContent(self, flag, id, vipflags):
        ...
        raw_url = f"{vod_url}?{ck}"
        if check_url:
            url = check_url
        else:
            url = self.proxyurl + '&url=' + quote(raw_url, safe='') if '.m3u8' in raw_url else vod_url
    ...
    def hema_m3u8_proxy(self, params):
        ...
        if params.get('format') == "ts":
            data = {"Location": self.hls_sign(url), "Content-Length": "0"}
    ...
    def hema_modify_m3u8(self, raw_url):
        ck = raw_url.split('?')[1]
        m3u8_url = self.hls_sign(raw_url)
    ...
    def hls_sign(self, url):
        ...
        sign_data = clean_url.replace(replaceDomain, self.encrypt_domain) + hex_time
        token = hashlib.md5(sign_data.encode('utf-8')).hexdigest()
        separator = '&' if '?' in url else '?'
        return f"{url}{separator}wsSecret={token}&wsTime={hex_time}"
