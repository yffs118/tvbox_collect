...
class Spider(Spider):
    ...
    def init(self,extend=""):
        ...
        self.ua="Mozilla/5.0"
        self.ok="http://192.168.2.3:8888"
        self.token="74b22034b792981b"
        self.search_api=self.ok+"/video/pansou-guangya"
    ...
    def homeContent(self,filter):
        ...
        cls += [{"type_id":"ri","type_name":"我的日更"},{"type_id":"okpan","type_name":"光鸭网盘"},{"type_id":"week","type_name":"周更列表"},{"type_id":"new","type_name":"最近更新"}]
        return {"class":cls}
    ...
    def detailContent(self,array):
        ...
        if sid.startswith("ok$"):
            ...
            vod={"vod_id":sid,"vod_name":name,"vod_pic":pic,"type_name":"光鸭网盘","vod_remarks":"已加入追更","vod_content":url,"vod_play_from":"光鸭网盘","vod_play_url":"推送播放$push$"+quote(url)}
            return {"list":[vod]}
    ...
    def searchContent(self,key,quick,pg="1"):
        ...
        if arr:
            first=arr[0]
            self.autoAdd(k,"zhui",first.get("vod_content",""),first.get("vod_pic",""))
        return {"list":arr,"page":int(pg),"pagecount":1,"limit":20,"total":len(arr)}
    ...
    def playerContent(self,flag,id,vipFlags):
        if id.startswith("push$"):
            ...
            return {"parse":0,"url":"push://"+u,"header":"User-Agent="+self.ua}
        if id.startswith("oksearch$"):
            ...
            if arr:
                ...
                return {"parse":0,"url":"push://"+u,"header":"User-Agent="+self.ua}
            return {"parse":1,"url":"https://guangyapan.fun/s/"+quote(k)+".html","header":"User-Agent="+self.ua}
    ...
    def okSearch(self,key):
        try:
            r=requests.get(self.search_api,params={"wd":key},headers={"User-Agent":self.ua,"token":self.token},timeout=15)
            js=r.json()
            res=[]
            for i,it in enumerate(js.get("list",[])[:30]):
                u=it.get("vod_id","")
                n=it.get("vod_name",key)
                p=it.get("vod_pic","")
                m=it.get("vod_remarks","")
                if not u or not self.isGuangyaPan(u):
                    continue
                res.append({"vod_id":"ok$"+quote(key)+"$"+quote(u)+"$"+quote(p),"vod_name":n,"vod_pic":p,"vod_remarks":m or "光鸭网盘","vod_content":u})
            return res
        except:
            return []

    def isGuangyaPan(self,u):
        return bool(re.search(r"(guangyapan\.(com|cn|fun|cc)|guangya\.(cc|cn|top|xyz)|api\.guangyapan\.com|/s/[A-Za-z0-9_-]+)",u or "",re.I))
    ...
    def autoAdd(self,name,gen,yurl="",pic=""):
        ...
        for it in arr:
            if it.get("name","").split(",")[0].split("|")[0]==name:
                if yurl and it.get("yurl","empty")=="empty":
                    it["yurl"]=yurl
