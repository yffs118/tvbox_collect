...
class Spider(Spider):
    def init(self, extend=""):
        r = random.random()
        suffix = '1' if r < 0.5 else ['2','3','4','5','6','7','8','9'][int((r-0.5)*16) % 8]
        self.host = f"https://www.mtyy{suffix}.com"
        self.headers = {
    ...
    def homeVideoContent(self):
        """Return recommended videos for homepage"""
        html = self.fetch_page(self.host)
        doc = pq(html) if html else pq('')
        videos = []
        for box in doc('.public-list-box.public-pic-b').items():
    ...
    def manualVideoCheck(self):
        """Perform manual video format validation"""
        return False
