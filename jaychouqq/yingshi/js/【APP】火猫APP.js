...
// === 全局配置信息（原代码保持不变）===
const HOST = "https://jp5.ggtvb.cc";
const HEADERS = {
  "User-Agent": "Dart/3.4 (dart:io)",
  "x-device-id": "b0d2ee192bc0c004",
  "x-app-package": "cc.xzsc.user",
  "x-app-signature": "11:05:5B:0E:DD:14:6A:96:38:28:CB:29:7D:5F:04:57:3F:D6:EC:6A:C4:54:4F:4D:44:72:0A:C1:8E:DD:A9:58",
  "x-app-version-code": "12",
  "x-app-version-name": "5.2"
};
...
const request = async (url, options = {}) => {
  try {
    ...
    console.error(`[火猫影视] Request error: ${url}`, e.message);
    return null;
  }
};
...
const _play = async ({ flag, flags, id, app }) => {
      ...
      for (const parserName of parserList) {
        ...
        if (funcName && app[funcName]) {
          try {
            ...
            console.error(`[火猫影视] ❌ 解析器 ${parserName} 失败:`, e.message);
          }
        } else {
          console.log(`[火猫影视] ⚠️ 解析器函数未找到: ${funcName}`);
        }
      }
      console.log('[火猫影视] 所有解析器失败，回退到原代码逻辑');
    }
    // 解析器都失败，继续走原代码的解密逻辑
  }
  ...
  try {
    ...
    console.error("[火猫影视] Play extraction error:", e.message);
  }
...
module.exports = async (app, opt) => {
  ...
  console.log(`[火猫影视] 智能线路版已加载`);
  console.log(`[火猫影视] 配置线路: ${Object.keys(sourceConfig.lines).join(', ')}`);
};