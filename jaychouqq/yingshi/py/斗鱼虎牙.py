...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def homeContent(self, filter):
		...
		try:
			...
			result['class'] = [{"type_id": 'douyu', "type_name": "斗鱼"},{"type_id": 'huya', "type_name": "虎牙"}]
			result['filters'] = {'douyu': {'key': '斗鱼', 'name': '斗鱼', "value": [{"n": "一起看", "v": "208"},{"n": "星秀", "v": "1008"},{"n": "二次元", "v": "174"}]},
                                 'huya': {'key': '虎牙', 'name': '虎牙', "value": [{"n": "一起看", "v": "getLiveList?iGid=2135"},{"n": "舞蹈", "v": "getTmpLiveList?iGid=1663&iTmpId=116"},{"n": "星秀", "v": "getLiveList?iGid=1663"},{"n": "二次元", "v": "getLiveList?iGid=2633"}]}}
		return result
		...
		if cid == 'bilibili':
			...
			if '虎牙' in ext:
				...
				try:
					...
					tid = 'getLiveList?iGid=2135'
			header['Referer'] = 'https://www.huya.com/'

			
			url = f'https://live.huya.com/liveHttpUI/{tid}&iPageSize=20&iPageNo={page}'
			#url = f'https://live.huya.com/liveHttpUI/getTmpLiveList?iGid=1663&iTmpId=116&iPageSize=20&iPageNo={page}'   
	...
	def detailContent(self, did):
		...
		if didList[1] == 'bilibili':
			...
			url = f'http://shorturl.jxpan.vip:880/r/baoshi_aio/douyu/{didList[2]}'
			platformList = ['斗鱼']
			playurlList = [f'直播${url}']
		elif didList[1] == 'huya':
			url = f'http://shorturl.jxpan.vip:880/r/baoshi_aio/huya/{didList[2]}'
			platformList = ['huya']
			playurlList = [f'直播${url}']
		else:
			playurlList = []
	...
	def searchbb(self, key, tag, pg, api):
		items = []
		header = self.header.copy()
		header['Cookie'] = 'buvid3=8802E18A-D264-86E5-AFF0-C0433CEE538C23554infoc'
	...
	def playerContent(self, flag, pid, vipFlags):
		result = {}
		header = {
		"User-Agent": "com.android.chrome/131.0.6778.200 (Linux;Android 13) AndroidXMedia3/1.8.0"
	}
