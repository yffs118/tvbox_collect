...
# ==================== 配置区域（可自行修改） ====================
# 缓存目录路径，默认为手机Music目录
CACHE_DIR = "/storage/emulated/0/Download/KuwoMusic/music/说唱"
    ...
    def _compress_cover_image(self, image_data):
        ...
        try:
            ...
            if width > self.cover_max_width or height > self.cover_max_height:
                ...
                print(f"封面缩放: {width}x{height} -> {new_width}x{new_height}")
            ...
            for quality in qualities:
                ...
                if len(compressed) <= self.cover_max_size_kb * 1024:
                    print(f"封面压缩: {original_size:.1f}KB -> {len(compressed)/1024:.1f}KB (质量{quality})")
                    return compressed
            ...
            compressed = buffer.getvalue()
            print(f"封面压缩: {original_size:.1f}KB -> {len(compressed)/1024:.1f}KB (最终)")
            return compressed
            
        except ImportError:
            # PIL不可用，只做简单处理
            print("PIL未安装，无法压缩图片，使用原图")
            return image_data
        except Exception as e:
            print(f"图片压缩失败: {e}，使用原图")
            return image_data
        ...
        if cover_url:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    print(f"封面下载失败: HTTP {resp.status_code}")
            except Exception as e:
                print(f"封面保存失败: {e}")
