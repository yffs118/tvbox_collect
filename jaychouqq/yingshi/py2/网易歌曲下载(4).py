...
# 封面压缩配置
COVER_MAX_SIZE_KB = 200      # 封面最大大小（KB）
COVER_MAX_WIDTH = 500        # 封面最大宽度（像素）
COVER_MAX_HEIGHT = 500       # 封面最大高度（像素）
# ================================================================
...
class Spider(Spider):
    def init(self, extend=""):
        ...
        self.cover_max_height = COVER_MAX_HEIGHT
    ...
    def _compress_cover_image(self, image_data):
        """压缩封面图片"""
        if len(image_data) <= self.cover_max_size_kb * 1024:
            return image_data
        ...
        try:
            ...
            if width > self.cover_max_width or height > self.cover_max_height:
                ratio = min(self.cover_max_width / width, self.cover_max_height / height)
            ...
            for quality in qualities:
                ...
                if len(compressed) <= self.cover_max_size_kb * 1024:
                    return compressed
        ...
        if cover_url:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    img_data = self._compress_cover_image(img_data)
                    with open(paths["cover"], 'wb') as f:
                        f.write(img_data)
                    cover_path = paths["cover"]
                    print(f"✓ 封面已保存 (大小: {len(img_data)/1024:.1f}KB)")
            except Exception as e:
                print(f"封面保存失败: {e}")
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        # 如果是下载请求
        if action == "download":
            print(f"📥 用户请求下载: {song_name} - {artist_name}")
            # 下载歌曲
            audio_path, cover_path, lrc_path = self._cache_song(song_id, song_name, artist_name, pic)
            if audio_path:
                # 返回一个提示信息（播放器会显示）
                return {
                    "parse": 0,
        ...
        if cached_audio:
            ...
            cover_display = cached_cover if cached_cover else pic
            return {
                "parse": 0,
        ...
        # 获取在线播放地址并播放（不自动下载，让用户手动点下载）
        print(f"在线播放: {song_name} - {artist_name}")
    ...
    def _build_play_urls(self, vod, songs):
        ...
        for s in songs:
            artists = [ar.get("name", "") for ar in s.get("ar", [])]
            name = f"{s.get('name', '')} - {'/'.join(artists)}"
            download_eps.append(f"{name}${s.get('id', '')}|download")
        play_urls.append("#".join(download_eps))
    ...
    def _build_single_song_detail(self, parts, singer_id):
        ...
        download_eps = []
        for s in singer_songs:
            download_eps.append(f"{s['name']} - {s['artist']}${s['id']}|download")
        play_urls.append("#".join(download_eps))
