...
# ==================== 本地文件协议头配置 ====================
# 本地文件协议前缀，根据播放器实际要求设置
LOCAL_FILE_PREFIX = "http://127.0.0.1:9978/file/"  # 根据你的播放器修改
# 或者直接使用 file:// 协议
# LOCAL_FILE_PREFIX = "file://"

# 如果需要本地音乐线路显示，可开启
ENABLE_LOCAL_MUSIC_LINE = True
# ===========================================================
...
class Spider(Spider):
    def init(self, extend=""):
        ...
        print("网易云音乐插件初始化完成")
        print(f"本地文件协议前缀: {LOCAL_FILE_PREFIX}")
    ...
    def _get_cache_audio_url(self, song_id, song_name):
        ...
        if cache_info and cache_info.get("audio_path") and os.path.exists(cache_info["audio_path"]):
            ...
            # 根据播放器要求，返回带协议头的本地地址
            if LOCAL_FILE_PREFIX.startswith("http://127.0.0.1") or LOCAL_FILE_PREFIX.startswith("http://localhost"):
                # HTTP方式访问本地文件服务器
                relative_path = audio_path.replace("/storage/emulated/0/", "")
                return f"{LOCAL_FILE_PREFIX}{relative_path}", audio_path
            else:
                # file:// 协议方式
                return f"file://{audio_path}", audio_path
        return None, None
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if cache_url:
            ...
            print(f"💿 命中缓存，直接播放本地文件: {os.path.basename(cache_path)} ({quality_info['name']})")
            ...
            if action == "download":
                return {
                    ...
                    "pic": pic or "",
                    "lrc": lrc_str or "",
                    "msg": f"✅ 文件已存在: {os.path.basename(cache_path)}"
                }
        ...
        if action == "download":
            ...
            # 下载完成后，返回本地缓存的URL（带协议头）
            cache_url, _ = self._get_cache_audio_url(song_id, song_display)
            final_url = cache_url if cache_url else play_url
