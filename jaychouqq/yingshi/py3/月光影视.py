...
class Spider:
    ...
    def _parse_list(self, html):
        ...
        results = []
        items = tree.xpath('//ul[@class="stui-vodlist clearfix"]//li')
        if not items:
            items = tree.xpath('//div[@class="stui-vodlist__box"]/..')
        if not items:
            items = tree.xpath('//li[contains(@class,"col-md-5")]')
        for item in items:
            try:
                ...
                title = a.get("title", "") or "".join(a.xpath('.//span[@class="pic-text"]/text()')).strip()
                if not title:
                    title_elem = item.xpath('.//h4[@class="title"]/a/text()')
                    if title_elem:
                        title = title_elem[0].strip()
                pic = a.get("data-original", "") or a.get("data-src", "") or a.get("src", "")
                remark = "".join(item.xpath('.//span[contains(@class,"pic-text")]/text()')).strip()
                if not remark:
                    remark_elem = a.xpath('.//span[contains(@class,"pic-text")]/text()')
                    if remark_elem:
                        remark = "".join(remark_elem).strip()
                results.append({
                    "vod_id": vod_id,
                    "vod_name": title,
