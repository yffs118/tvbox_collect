...
export default {
  async fetch(request) {
    ...
    try {
      ...
      /* ===== M3U 订阅路由 ===== */
      if (path === '/love.m3u')    return handleM3U(params, url);
      if (path.match(/^\/love-(.+)\.m3u$/)) {
        const seg = path.match(/^\/love-(.+)\.m3u$/)[1];
        // /love-{category}.m3u  or  /love-daily.m3u  /love-weekly.m3u  /love-monthly.m3u
        const rangeKeys = { daily: 'today', weekly: 'week', monthly: 'month' };
        if (rangeKeys[seg]) {
          params.set('range', rangeKeys[seg]);
        } else {
          params.set('category', seg);
        }
        return handleM3U(params, url);
      }
      ...
      if (path === '/search') {
        ...
        return handleList({ sort: 'favorite', category: '', q: q, page: page, isSearch: true, origin: url.origin });
      }
      const catMatch = path.match(/\/category\/([^\/\s?]+)/);
      if (catMatch) {
        const page = parseInt(params.get('page') || '1', 10);
        return handleList({ sort: 'favorite', category: catMatch[1], page: page, origin: url.origin });
      }
      const sortMap = {
      ...
      const page = parseInt(params.get('page') || '1', 10);
      return handleList({ sort: sort, category: '', page: page, origin: url.origin });
    } catch (e) {
 ...
 * M3U 订阅生成
 * 访问方式：
 *   /love.m3u                      全部视频（今日，前3页）
 *   /love.m3u?range=week           本周
 *   /love.m3u?range=month          本月
 *   /love.m3u?range=all            全部时段
 *   /love.m3u?category=anime       动漫分类
 *   /love.m3u?pages=5              抓取页数（1-10）
 *   /love-daily.m3u                今日快捷
 *   /love-weekly.m3u               本周快捷
 *   /love-monthly.m3u              本月快捷
 *   /love-anime.m3u                分类快捷
 * ================================================================ */
async function handleM3U(params, reqUrl) {
  const range    = params.get('range')    || 'today';
  const category = params.get('category') || '';
  const q        = params.get('q')        || '';
  const maxPages = Math.min(Math.max(parseInt(params.get('pages') || '3', 10), 1), 10);
  const rangeToSort = { today: 'daily', week: 'weekly', month: 'monthly', all: 'favorite' };
  const sort = rangeToSort[range] || 'daily';
  const pageNums = [];
  for (let i = 1; i <= maxPages; i++) pageNums.push(i);
  const results = await Promise.all(
    pageNums.map(function(p) {
      return apiList({ sort: sort, category: category, q: q, page: p }).catch(function() { return null; });
    })
  );
  const allMovies = [];
  const seen = {};
  ...
  if (category) {
    const cat = CATEGORIES.find(function(c) { return c.id === category; });
    groupTitle = cat ? cat.name : category;
  } else if (range === 'today')  { groupTitle = '今日'; }
  else if (range === 'week')     { groupTitle = '本周'; }
  else if (range === 'month')    { groupTitle = '本月'; }
  const lines = ['#EXTM3U x-tvg-url=""'];
  for (const m of allMovies) {
    if (!m.videoUrl) continue;
    const title = m.title.replace(/,/g, '，');
    lines.push(
      '#EXTINF:-1 tvg-id="' + m.id + '" tvg-name="' + title + '" tvg-logo="' + m.thumb + '" group-title="' + groupTitle + '" duration="' + m.time + '",' + title
    );
    lines.push(m.videoUrl);
  }
  const body = lines.join('\n') + '\n';
  const filename = category ? ('love-' + category + '.m3u') : ('love-' + range + '.m3u');
...
'</div>' : '';
  ...
  '<div class="m3u-section">' +
    '<div class="m3u-section-head">' +
      '<span class="m3u-section-title">📺 M3U 播放列表</span>' +
      '<span class="m3u-section-tip">适用于电视盒子 / Kodi / IPTV 播放器</span>' +
    '</div>' +
    '<div class="m3u-links">' +
      '<a href="' + origin + '/love.m3u?range=today&pages=5"   class="m3u-btn" download>🔥 今日</a>' +
      '<a href="' + origin + '/love.m3u?range=week&pages=5"    class="m3u-btn" download>📅 本周</a>' +
      '<a href="' + origin + '/love.m3u?range=month&pages=5"   class="m3u-btn" download>🗓 本月</a>' +
      '<a href="' + origin + '/love.m3u?range=all&pages=5"     class="m3u-btn" download>✨ 全部</a>' +
    '</div>' +
    (category ? '<div class="m3u-cat-row"><a href="' + origin + '/love-' + category + '.m3u?pages=5" class="m3u-btn m3u-btn-cat" download>📁 当前分类：' + esc(catLabel || category) + '</a></div>' : '') +
    '<div class="m3u-copy-row">' +
      '<span class="m3u-copy-label">订阅地址：</span>' +
      '<input type="text" class="m3u-copy-input" id="m3uUrlInput" readonly' +
        ' value="' + origin + (category ? ('/love-' + category + '.m3u?pages=5') : '/love.m3u?range=today&pages=5') + '">' +
      '<button class="m3u-copy-btn" onclick="copyM3UUrl()">复制</button>' +
    '</div>' +
  '</div>' +
...
'}' +
'function copyM3UUrl() {' +
'  var inp = document.getElementById("m3uUrlInput");' +
'  if (!inp) return;' +
'  var full = inp.value;' +
'  if (navigator.clipboard && navigator.clipboard.writeText) {' +
'    navigator.clipboard.writeText(full).then(function() {' +
'      _m3uCopied();' +
'    }).catch(function() { _m3uFallback(full); });' +
'  } else { _m3uFallback(full); }' +
'}' +
'function _m3uFallback(text) {' +
'  var ta = document.createElement("textarea");' +
'  ta.value = text; ta.style.position = "fixed"; ta.style.opacity = "0";' +
'  document.body.appendChild(ta); ta.select();' +
'  try { document.execCommand("copy"); _m3uCopied(); } catch(e) {}' +
'  document.body.removeChild(ta);' +
'}' +
'function _m3uCopied() {' +
'  var btn = document.querySelector(".m3u-copy-btn");' +
'  if (!btn) return;' +
...
'.m3u-section{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:16px 20px;margin-bottom:20px}' +
'.m3u-section-head{display:flex;align-items:baseline;gap:10px;margin-bottom:14px;flex-wrap:wrap}' +
'.m3u-section-title{font-size:14px;font-weight:700;color:var(--text)}' +
'.m3u-section-tip{font-size:11px;color:var(--muted)}' +
'.m3u-links{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}' +
'.m3u-btn{display:inline-flex;align-items:center;gap:5px;padding:8px 16px;border-radius:8px;font-size:12px;font-weight:700;background:var(--surface2);border:1px solid var(--border);color:var(--text);transition:all .2s;white-space:nowrap;text-decoration:none;-webkit-tap-highlight-color:transparent;}' +
'.m3u-btn:hover{border-color:rgba(29,155,240,.5);color:#1d9bf0}' +
'.m3u-btn-cat{background:rgba(29,155,240,.1);border-color:rgba(29,155,240,.3);color:#1d9bf0}' +
'.m3u-cat-row{margin-bottom:10px}' +
'.m3u-copy-row{display:flex;align-items:center;gap:8px;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:6px 8px 6px 12px}' +
'.m3u-copy-label{font-size:11px;color:var(--muted);white-space:nowrap;flex-shrink:0}' +
'.m3u-copy-input{flex:1;background:transparent;border:none;outline:none;color:var(--muted);font-size:12px;font-family:monospace;min-width:0}' +
'.m3u-copy-btn{flex-shrink:0;background:var(--accent);color:#fff;border:none;padding:6px 14px;border-radius:6px;font-size:12px;font-weight:700;cursor:pointer;transition:background .15s}' +
'.m3u-copy-btn:hover{background:var(--accent2)}' +
