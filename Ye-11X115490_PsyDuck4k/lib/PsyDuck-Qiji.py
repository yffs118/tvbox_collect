...
class Spider(Spider):
    ...
    def init(self, extend=''):
        ...
        api = str(ext.get('api', 'getappapi'))
        if api == '2' or api == 'qiji':
            self.get_type = 'qiji'
            api_path = '/api.php/qijiappapi'
        elif api == 'flutter':
            self.get_type = 'flutter'
            api_path = '/api.php/getappapi'
        else:
            self.get_type = 'get'
            api_path = '/api.php/getappapi'
        ...
        host_source = ''
        if 'host' in ext:
            host_source = ext['host']
        elif 'url' in ext:
            host_source = ext['url']
        elif 'site' in ext:
            host_source = ext['site']
        
        if not host_source:
            print("错误: 需要提供host、url或site参数")
            return
            
        domain_set = set()
        
        if not re.match(r'^https?://[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(:\d+)?(/)?$', host_source):
            try:
                host_data = self.fetch(host_source, headers=self.get_headers(1), timeout=10, verify=False).text
                domain_set.update([domain.strip() for domain in host_data.split('\n') if domain.strip()])
            except Exception as e:
                print(f"获取站点文件失败: {e}")
                return
        else:
            domain_set.add(host_source.rstrip('/'))
        ...
        self.device_id = ext.get('deviceId') or ext.get('devideid') or '1234567890ABCDEF'
        self.version = str(ext.get('version', '1.0.0'))
        
        self.key = ext.get('key') or ext.get('datakey') or ext.get('dataKey')
        if not self.key:
            print("错误: 需要提供加密密钥(key/datakey/dataKey)")
            return
        
        self.iv = ext.get('iv') or ext.get('dataiv') or ext.get('dataIv') or self.key
